    ...
    * Functions as below assigned to pins
    ...
    void setup()
    {
        ...
        ether.udpServerListenOnPort(&udpSteerRecv, 8888);
		    ...
		    // PCA9685
        Wire.begin(); //I2C pins on SCL A5, SDA A4
        Wire.setClock(400000); //Increase I2C data rate to 400kHz
  		  ...
  		  if (PCA9685_found)
	    	{
  	  		Serial.println("OK_PCA9685 expander found.");
...
delay(1000);
    ...
    void scanI2C() {
      Serial.println("Scanning for I2C devices on the bus...");
      byte error, address;
      int devicesFound = 0;
...
//        if (pin[0]) digitalWrite(4, relayState[pin[0] - 1]);
        ...
        for(int i = 0 ; i <= 8 ; i++)
        {
          if(pin[i] && PCA9685_found){
            if(relayState[pin[i] -1])
            {
              PWMServoDriver.setPWM(8-i, 4096, 0);
            } 
            else 
            {
              PWMServoDriver.setPWM(8-i, 0, 4096);
            }
          }
        }
    }
